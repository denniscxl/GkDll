...
namespace GKBase
{
    public class GKEditor
    {
        ...
        static public T LoadOrCreateAssetByNonResource<T>(string filename) where T : UnityEngine.Object
        {
            if(System.IO.File.Exists(filename))
            {
                return AssetDatabase.LoadAssetAtPath(filename, typeof(T)) as T;
            }
       ...
       // [MenuItem("GK/File Tools/Delete Empty Folders")]
        ...
        static public List<string> DeleteEmptyFolders(string folder)
        {
            var list = new List<string>();
        ...
        static public string GetAssetDirectoryFromSelection()
        {
            var outpath = "Assets";
        ...
        static public bool CreateNewPrefabInAssets<T>() where T : Component
        {
        ...
        static public bool CreateNewScriptableObjectInAssets<T>() where T : ScriptableObject
        {
            var t = typeof(T);
        ...
        static public string GetCreateNewAssetFromSelectionFilename(string filenameSample)
        {
        ...
        static public Texture2D CreateReadableTexture(Texture tex)
        {
            if (!tex) return null;
        ...
        static public bool FileUpdateNeeded(string dstFile, string srcFile)
        {
        ...
        static public void DrawInspectorSeperator()
        {
        ...
        static public void DrawMiniInspectorSeperator()
        {
            var rc = GUILayoutUtility.GetRect(1, 1);
            var c = 0.1f;
            EditorGUI.DrawRect(rc, new Color(c, c, c));
        ...
        static public int GetGreenwichTime()
        {
            System.DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1));
            return (int)(DateTime.Now - startTime).TotalSeconds;
        }

        static public void DrawArrow(Vector2 from, Vector2 to, Color color)
        {
            Handles.BeginGUI();
            Handles.color = color;
        ...
        static public void DrawBaseControl(bool enable, object value, Action<object> setValue)
        {
            object tVal;
            if (value is Enum)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.EnumPopup(value as Enum);
                    if ((Enum)tVal != (Enum)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Bounds)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.BoundsField((Bounds)value);
                    if ((Bounds)tVal != (Bounds)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Color)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.ColorField((Color)value);
                    if ((Color)tVal != (Color)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is AnimationCurve)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.CurveField((AnimationCurve)value);
                    if ((AnimationCurve)tVal != (AnimationCurve)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is string)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.TextField(value as string);
                    if ((string)tVal != (string)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is float)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.FloatField((float)value);
                    if ((float)tVal < (float)value - 0.01f || (float)tVal > (float)value + 0.01f)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is double)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.DoubleField((double)value);
                    if ((double)tVal < (double)value - 0.01f || (double)tVal > (double)value + 0.01f)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is int)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.IntField((int)value);
                    if ((int)tVal != (int)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is long)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.LongField((long)value);
                    if ((long)tVal != (long)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is bool)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.Toggle((bool)value);
                    if ((bool)tVal != (bool)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Vector2)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.Vector2Field("", (Vector2)value);
                    if ((Vector2)tVal != (Vector2)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Vector3)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.Vector3Field("", (Vector3)value);
                    if ((Vector3)tVal != (Vector3)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Vector4)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.Vector4Field("", (Vector4)value);
                    if ((Vector4)tVal != (Vector4)value)
                        setValue(tVal);
                }
                else
                {
            ...
            else if (value is Rect)
            {
                if (enable)
                {
                    tVal = EditorGUILayout.RectField("", (Rect)value);
                    if ((Rect)tVal != (Rect)value)
                        setValue(tVal);
                }
                else
                {
			...
			else if(value is UnityEngine.Object)
			{
				if (enable)
				{
                    tVal = EditorGUILayout.ObjectField((UnityEngine.Object)value, value.GetType(), true);
                    if (tVal != value)
                        setValue(tVal);
				}
				else
